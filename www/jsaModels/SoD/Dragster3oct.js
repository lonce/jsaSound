...
define(
	...
	function (config, baseSM, fmodOscFactory, jsaConvolverFactory, GraphNode) {
        return function (i_loadedCB) {
            ...
            // for note number input parameter
            var m_basefreq = 65.4;// CHEAT, shift down 2 octaves from specified midi number 261.62 ; // C3, MIDI note 48, octave below middle C 
            var m_lowNote = 60;
            var m_hiNote = 96; //(inclusive) C6, MIDI note 84, 2 octaves above middle C
            m_noteNum = 72;
...
*/
            myInterface.registerParam(
                    ...
                    function (i_val) {
                        m_noteNum=i_val;
                        m_car_freq=m_basefreq*Math.pow(2, (i_val-m_lowNote)/12); // audioUtils.note2Freq(notes[0][Math.floor(i_val)])
                        console.log("nom Freq is " + m_car_freq);
